...
import json
import math
import streamlit.components.v1 as components  # Using Streamlit's native low-level component engine
...
# --- DIRECT HTML5 PERMISSIONS-PASSING GEOLOCATION COMPONENT ---
st.markdown("### 📡 Device Location Synchronizer")

# Catch query parameters passed back from the browser injection
...
# Native Browser JS Geolocation script injected cleanly via iframe bridge
gps_html = """
<div style="text-align: center; font-family: sans-serif;">
    ...
    ">🔗 Synchronize My Mobile GPS</button>
    <p id="status" style="color: #94a3b8; font-size: 13px; margin-top: 8px;">Not connected</p>
</div>
...
<script>
    ...
    button.addEventListener('click', () => {
        status.innerText = "Requesting satellite link...";
        if (!navigator.geolocation) {
            status.innerText = "Error: Geolocation not supported by your browser.";
            return;
        }
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                status.innerText = "Success! Reloading matrix...";
                
                // Bust open out of the iframe cage and update parent window params
            ...
            (error) => {
                if(error.code === 1) {
                    status.innerText = "Permission Denied. Unlock site location access in settings.";
                } else {
                    status.innerText = "GPS Error: " + error.message;
                }
            },
...
# Render the macro-component securely
...
if st.session_state.geo_synced:
    ...
    st.warning("ℹ️ Running on default Subang Jaya location matrix. Click the Synchronize button above to authorize live tracking.")
