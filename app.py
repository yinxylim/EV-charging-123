...
# --- BULLETPROOF ANDROID GPS COUPLER ---
...
if user_lat_param and user_lon_param:
    ...
    st.warning("📱 Android Iframe Blocked: Mobile browsers require a secure top-level window redirect to share GPS data safely.")
    
    # We embed a pure, beautiful HTML button that targets the top-level parent window directly, bypassing cross-origin blocks.
    sync_html = """
    <div style="text-align: center; margin-bottom: 20px;">
        <button onclick="getMobileLocation()" style="
            background: linear-gradient(90deg, #38bdf8, #818cf8);
            color: white;
            border: none;
            padding: 12px 24px;
        ...
        function getMobileLocation() {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(function(position) {
                    ...
                    // TARGET THE MAIN WINDOW INSTEAD OF THE STREAMLIT SANDBOX
                    ...
                    alert("GPS Error: Please ensure Location permissions are turned on for this browser tab in your Android settings.");
                }, {enableHighAccuracy: true, timeout: 8000});
            } else {
                alert("Your browser does not support GPS location services.");
            }
        }
    </script>
    """
    components.html(sync_html, height=60)
