...
# --- ANDROID MOBILE GPS COUPLER ---
query_params = st.query_params
user_lat_param = query_params.get("lat")
user_lon_param = query_params.get("lon")

if user_lat_param and user_lon_param:
    st.session_state.user_lat = float(user_lat_param)
    st.session_state.user_lon = float(user_lon_param)
    ...
    st.info("📱 Android Notice: Mobile browsers block background location tracking. Use the button below to sync.")
    
    if st.button("🎯 Sync My Live Location"):
        location_html = """
        <script>
            navigator.geolocation.getCurrentPosition(function(position) {
                ...
                alert("GPS Error: Please enable location accuracy in your Android quick settings!");
...
# --- EXPANDED MALAYSIAN COMPREHENSIVE EV VEHICLE DATABASE ---
EV_MODELS = {
    # --- National Brands ---
    ...
    # --- Premium European (BMW, Volvo, Mercedes, Porsche) ---
    ...
    "Porsche Taycan (Base 2026)": {"battery_kwh": 89.0, "max_ac_kw": 22.0, "max_dc_kw": 270.0},
    
    # --- Generic Fallback ---
    "Custom / Other Vehicle Specs": {"battery_kwh": 60.0, "max_ac_kw": 11.0, "max_dc_kw": 100.0}
}

# Regional Charging Station Database (Coordinates mapped dynamically via Haversine Formula)
STATIONS = [
...
if not results:
    st.error("❌ No charging hubs match your session parameters. Try loosening your cost/time ceiling constraints.")
else:
...
st.markdown("---")
st.markdown("<p style='text-align: center; color: #64748b;'>⚡ Complete Malaysian EV Model Matrix. Safe GPS sorting verified on mobile platforms.</p>", unsafe_allow_html=True)
