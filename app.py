...
# 1. PAGE CONFIG & MODERN CUSTOM THEME INJECTION
st.set_page_config(
...
# Custom CSS for a vibrant, modern tech appearance
st.markdown("""
    <style>
    ...
    div.element-container stActionButton {
        background-color: #3b82f6 !important;
    }
    
    /* Make Metric Blocks Look Premium */
    ...
    /* Custom Styling for Expander Headers */
    .stExpander {
...
# --- MAIN SCREEN INPUTS (No hidden sidebar needed, better for fast mobile use) ---
...
# Expandable Fine-Tuning Box (Keeps screen clean, lets geeks customize)
...
st.write("---")
st.markdown(f"📊 **Target Assessment:** Adding **{soc_delta}%** charge requires ~**{energy_needed_theoretical:.2f} kWh** of raw energy.")
...
if not results:
    ...
    for idx, charger in enumerate(results, 1):
        # Colorful tag indicators based on completion
        ...
        with st.expander(f"#{idx}: {charger['name']} [{charger['type']} {charger['power']}] — RM {charger['cost']:.2f}"):
            ...
            # Bright visible action link button
            st.markdown(f"🎯 **[Tap to Open Navigation Map & Station Rules]({charger['link']})**")

st.markdown("---")
# Engaging tip jar footer to attract organic revenue support
