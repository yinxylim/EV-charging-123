...
# --- JAVASCRIPT GEOLOCATION COUPLER ---
# This hidden component fetches browser GPS coords and sends them back to Streamlit
st.caption("🌐 Location Engine Activation")
location_html = """
<script>
    navigator.geolocation.getCurrentPosition(function(position) {
        ...
        // Send data back to Streamlit parent frame via window query string
        const urlParams = new URLSearchParams(window.parent.location.search);
        if (urlParams.get('lat') !== coords.lat.toString()) {
            window.parent.location.href = window.parent.location.pathname + '?lat=' + coords.lat + '&lon=' + coords.lon;
        }
    }, function(error) {
...
# Dynamically set title subtitle based on user location state
if user_lat and user_lon:
    user_lat = float(user_lat)
    user_lon = float(user_lon)
    location_title = "Live GPS Matcher & Smart Curve Simulator"
    st.success(f"📍 GPS Sync Active: Showing closest stations to your current position ({user_lat:.4f}, {user_lon:.4f})")
else:
    # Fallback default coordinates (Subang Jaya Centerpoint) if user denies permission
    user_lat = 3.0792
    user_lon = 101.5834
    location_title = "Subang Jaya Station Matcher & Smart Curve Simulator"
    st.info("💡 Tip: Allow browser location permissions to automatically unlock real-time tracking across Malaysia.")
...
# Regional Charging Database with Coordinates Added (lat, lon)
STATIONS = [
...
# Mathematical helper to figure out straight-line distance (Haversine Formula)
...
for station in STATIONS:
    # Compute live distance to this station
...
st.markdown("---")
st.markdown("<p style='text-align: center; color: #64748b;'>⚡ Built by an independent local developer. Tracks your location in real time across Malaysia! ☕</p>", unsafe_allow_html=True)
