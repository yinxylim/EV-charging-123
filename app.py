...
with col_state:
    selected_state = st.selectbox(
        ...
        states, 
        index=states.index("Kedah") if "Kedah" in states else 0
    )
...
battery_capacity = st.number_input("Pack Useable Capacity (Net kWh)", min_value=10.0, value=62.5, step=0.5)
...
# --- 🎯 STEP 3: CHARGING CONSTRAINTS ORGANISED HERE ---
...
with c_col2:
    max_wait_time = st.number_input("Max Time Allowance (Minutes)", min_value=10, max_value=480, value=180, step=10)
...
for station in LIVE_STATIONS:
    ...
    # Check dual constraints: Budget Cap AND Time Allowance Window
